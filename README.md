...
> **声明**：本项目是通过 AI Vibe Coding（[TRAE](https://www.trae.ai/) + GLM）开发的。
...
- **自动翻页**：TXT/PDF 均支持，点击按钮开启/停止，长按可选间隔（5–60 秒），读到书末自动停止

- **阅读设置**：字号、行距、页边距、主题（含夜间），即改即生效并持久化

- **续读**：TXT 记录章节+字符偏移，PDF 记录页码，重新打开自动定位

- **书架**：按最近阅读时间排序，最近读过的排在最前
...
同步下载的书籍存放于「文档/sync\_incoming」；自行打开的书保持原位置不动。
...
- Android Release APK 使用正式签名（v1.1.0 起）。签名密钥与 `key.properties` 不入库，构建时放好 `android/key.properties` 即自动启用正式签名，缺失则回退 debug 签名
